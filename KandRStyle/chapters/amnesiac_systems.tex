...
We found an article which examined the types of problems with blockchains in detail.  The authors classified the problems into four main categories: scalability limits arising from the cost of validating an ever-growing chain; energy and computational overhead from consensus mechanisms; governance disputes over how (or whether) an immutable ledger can be amended after deployment; and privacy conflicts, such as the \gls{gdpr} tension described above, where legally mandated mutability collides with a system designed never to forget.\endnote{Zheng, Zibin, et al. "Blockchain Challenges and Opportunities: A Survey." International Journal of Web and Grid Services, vol. 14, no. 4, 2018, pp. 352--375, \url{https://doi.org/10.1504/IJWGS.2018.095647}.}  These categories echo the trade-offs we faced in our own design: an amnesiac \gls{operating system} that is aggressively immutable gains resistance to tampering, but at the cost of losing the very audit trail an administrator might need after an incident, unless that trail is deliberately relocated to mutable, off-system storage.

\subsubsection{Immutability Examples in Database Administration}

Database administrators have long used append-only and write-once patterns for similar reasons.  Write-once, read-many (\index{WORM}WORM) storage has been used for regulatory retention of financial and healthcare records, where the goal is to prevent an insider or attacker from silently altering a historical record.\endnote{Menon, Jai, and Larry Stockmeyer. "An Age-Threshold Algorithm for Garbage Collection in Log-Structured Arrays and Applications to WORM Files." Proceedings of the 10th International Conference on Computer Performance Evaluation: Modelling Techniques and Tools, Springer, 1998, pp. 119--133, \url{https://doi.org/10.1007/BFb0057725}.}  More recently, append-only transaction logs in database engines and \gls{database} replication systems serve a similar function: mutable "current state" views are reconstructed by replaying an immutable sequence of writes, which lets administrators audit or roll back to any earlier point without ever having permitted an in-place overwrite of history.\endnote{Kleppmann, Martin. Designing Data-Intensive Applications: The Big Ideas Behind Reliable, Scalable, and Maintainable Systems. O'Reilly Media, 2017, pp. 453--460.}  This pattern is instructive for our \gls{amnesiac} design: rather than making the whole system immutable, we can, as in database log-structured storage, keep an immutable "truth" (the \gls{os} image) while allowing bounded, auditable mutability in a clearly separated layer (attached working storage).

\subsection{Related Amnesiac and Immutable-\gls{os} Approaches}
Our design is not the first attempt to build a system that forgets on purpose.  \index{Tails}Tails, the Debian-based live \gls{operating system} maintained by the Tor Project, is explicitly designed to leave no trace on the host hardware between boots by running entirely from removable media and \gls{ram}, with persistence only available through an optional, separately encrypted volume that a user must deliberately enable.\endnote{Tails Project. "About Tails." Tails.Boum.Org, 2026, \url{https://tails.net/about/}. Accessed 13 July 2026.}  Container-oriented immutable node designs, such as those used for Kubernetes worker nodes built from a read-only base image (for example, Flatcar Container Linux or Fedora CoreOS), take a related but distinct approach: the host \gls{operating system} partition is mounted read-only and updated only as a whole, atomic image, while workloads run in ephemeral containers whose state is not expected to persist across a rebuild.\endnote{Fedora CoreOS Project. "Fedora CoreOS Documentation." Docs.Fedoraproject.Org, 2026, \url{https://docs.fedoraproject.org/en-US/fedora-coreos/}. Accessed 13 July 2026.}  Desktop-oriented immutable distributions such as Fedora Silverblue and NixOS take a third approach, using atomic, transactional updates and rollback of an underlying image or store while still permitting substantial user-level customization through layered or declarative package management.\endnote{Fedora Project. "Fedora Silverblue." Silverblue.Fedoraproject.Org, 2026, \url{https://silverblue.fedoraproject.org/}. Accessed 13 July 2026.}\endnote{NixOS Project. "NixOS 25.05 Manual." Nixos.Org, 2026, \url{https://nixos.org/manual/nixos/stable/}. Accessed 13 July 2026.}

These systems differ from our proposed design chiefly in the mechanism used to enforce immutability and in what is treated as separable from the immutable core.  Tails relies on the ephemerality of \gls{ram} and a read-only image, much like our approach, but is intended for a single session on commodity end-user hardware rather than a persistent multi-user server hosting long-running \glspl{jail}.  The container-node and desktop-immutable approaches rely on filesystem-level read-only mounts and atomic image swaps enforced entirely in software; by contrast, our design layers a \index{physical security}physically enforced form of \gls{hard immutability} (an optical disc that cannot be rewritten without physical access and a new burn) underneath the same class of software controls, on the theory that software-only enforcement can potentially be defeated by an attacker with sufficient privilege escalation, whereas a write-once physical medium cannot be rewritten by any software-only \gls{attack}.  This distinction is a candidate contribution of our approach, though, as discussed in Section~\ref{sec:amnesiac-discussion}, we have not yet empirically compared the two enforcement strategies under a common \gls{attack} model.

\subsection{Comparisons of Technologies or Frameworks}
\subsubsection{Immutability and \gls{biba}}
\index{mac!biba} \Gls{biba} is a mandatory access control scheme, similar to military multilevel security classifications, but with an emphasis on file \gls{integrity}. \texttt{mac\_biba} allows for write down and read up; this is the opposite direction from mandatory access control schemes like military security classifications which can write up and read down. \endnote{Martins, Gary R., et al. Implementing Message Systems in Multilevel Secure Environments: Problems and Approaches. Rand, 1982.}\endnote{Biba, K J, et al. \Gls{integrity} Considerations for Secure Computer Systems. 1977.} FreeBSD offers \texttt{mac\_biba} as part of its \gls{operating system}; unfortunately, OpenBSD does not.  \endnote{"Mac\_Biba(4)." Freebsd.Org, 2025, \url{https://man.freebsd.org/cgi/man.cgi?query=mac_biba&sektion=4&apropos=0&manpath=freebsd+15.1-release+and+ports}. Accessed 13 July 2026.}  \Gls{biba} and our \gls{amnesiac} design share a structural idea even though they operate at different layers: both separate a trusted, low-mutability core from an untrusted or higher-mutability periphery, and both rely on a directional rule (write-down/read-up in \gls{biba}; disc-to-\gls{ram}-only in our design) to keep contamination from flowing from the less trusted layer back into the trusted one.

\subsection{Gaps in the Literature}
The literature we reviewed establishes that immutability is a well-studied idea in blockchains, databases, and mandatory access control, and that several production systems (Tails, immutable container nodes, Silverblue, NixOS) already implement software-enforced amnesiac or read-only behavior.  However, we found three gaps directly relevant to our design.  First, none of the sources we reviewed empirically measure how much additional resistance a \emph{physically} enforced read-only medium (such as an optical disc that must be re-burned to change) provides over software-only read-only mounts, against a realistic FreeBSD-hosted \gls{attack} scenario; this is a gap we return to as a candidate for future empirical work rather than something we resolve in this chapter.  Second, the literature on immutability's operational costs (Vppalapati; \gls{gdpr} right-to-be-forgotten conflicts) is not connected in prior work to \index{ZFS}\gls{zfs}-based server infrastructure specifically, leaving open how administrators should partition immutable and mutable \gls{zfs} datasets on a single host.  Third, we did not find published work that maps \gls{amnesiac} or immutable-\gls{os} designs directly onto the \index{MITRE ATT\&CK}MITRE ATT\&CK technique groups they are intended to mitigate, which is the mapping we attempt in Tables \ref{tab:mitrewrite1}--\ref{tab:mitrewrite6} above; our mapping is a first pass and has not been validated through red-team testing.

\section{Methodology}
\subsection{Technical Research Questions}
\subsection{System Architecture and Design}
\subsection{Implementation Plan}
We're starting with a Dell T610 Server.  Manufactured in 2011, this system is representative of decommissioned enterprise hardware available through government e-waste auctions, where it was acquired for this study.  Prior to configuration, the chassis required physical cleaning to remove accumulated dust from its cooling paths.
...
For reference throughout this chapter, the test system is designated by the hostname "Blondie."\endnote{“Blondie | the Official Website of Blondie, Featuring Tour Dates, Presale Ticketing, News, the Official Store and More.” \url{Blondie.net}, 2026, blondie.net/. Accessed 10 June 2026.}
...
\begin{itemize}
	\item A \gls{dvd} \gls{rw} drive
    	...
    	\item In its 8 drive bay, we have:
	\begin{itemize}
...
\texttt{\index{zpool!create}zpool create \gls{raid} \seqsplit{POOL\_NAME} \seqsplit{DISK\_LABEL}/\gls{zfs}\_\gls{partition}\_\seqsplit{LABEL} ...\seqsplit{next} disk/\gls{partition}...}\endnote{"zpool." Freebsd.org, 2025, \url{man.freebsd.org/cgi/man.cgi?query=zpool&apropos=0&sektion=0&manpath=FreeBSD+10.3-RELEASE&arch=default&format=html}. Accessed 10 June 2026.}\\\\
...
\section{Analysis}
\subsection{Quality}
This chapter's contribution is best characterized as a design-level, qualitative analysis rather than a validated empirical study, and we state that limitation explicitly here rather than implying otherwise.  We evaluate the design against the hypotheses in Section \ref{sec:scope-of-work} by tracing, for each hypothesis, whether the implemented \gls{configuration} plausibly satisfies it given the mechanisms actually built, and by cross-checking those mechanisms against the MITRE ATT\&CK technique groups identified in Tables \ref{tab:mitrewrite1}--\ref{tab:mitrewrite6}.

For \textbf{H1} ("Attackers cannot destructively interact with systems without writing to them"), the technique survey in Tables \ref{tab:mitrewrite1}--\ref{tab:mitrewrite6} supports the premise for the sampled Enterprise techniques we reviewed, but the survey was conducted by the authors without independent verification or inter-rater review, so it should be read as an initial classification rather than an exhaustive or independently validated one.

For \textbf{H2} ("An immutable \gls{operating system} will be more resistant to destructive \gls{attack}"), the Blondie build does implement \gls{hard immutability} for the core \gls{os} (optical media, loaded read-only into \gls{ram} at boot) as designed.  This is a structural argument, not a measured one: we have not subjected Blondie to a red-team \gls{attack} campaign, nor compared its resistance to a baseline mutable FreeBSD install under identical conditions.  Until such testing is performed, H2 should be treated as supported only at the level of design intent.

For \textbf{H3} ("An immutable \gls{operating system} will promote system \gls{integrity}"), the design choice to keep \glspl{jail} on separately mountable, disconnectable \gls{zfs} kits (Section \ref{sec:zfs-config}) is consistent with limiting the blast radius of a compromised \gls{jail} from reaching the core \gls{os}.  However, we have not measured \gls{integrity} outcomes (for example, whether file hashes of the core \gls{os} remain unchanged after a simulated \gls{ransomware} run against a \gls{jail}), so this hypothesis also remains a design-level claim pending empirical confirmation.

For \textbf{H4} ("An immutable \gls{operating system} will promote rapid recovery"), the amnesiac boot sequence (Section \ref{sec:amnesiac-boot}) is designed so that a power cycle restores the core \gls{os} to a known state without requiring a restore-from-\gls{backup} operation.  We have not, however, measured recovery time against a conventional \gls{backup}-and-restore baseline, so we can report only that the mechanism exists, not that it is faster in practice.

\subsection{Tests}
No controlled experiments, red-team exercises, or timed recovery trials were conducted as part of this chapter; the "Blondie" build documented here is a single instance rather than a study population, and its behavior has not been compared against a control system.  We view formal testing (see Section \ref{sec:amnesiac-discussion}) as the most important next step for validating H1--H4 and, more broadly, for elevating this material from a documented case study to an empirically supported research contribution.

\section{Discussion}
\label{sec:amnesiac-discussion}
This chapter presented the design rationale, threat-model grounding, and build procedure for an \gls{amnesiac} FreeBSD system, but it stops short of empirically testing the hypotheses it proposes.  Read together with Section \ref{sec:scope-of-work}, the case for H1--H4 rests on (a) a threat-model argument that a large share of MITRE ATT\&CK Enterprise techniques require a write action, and (b) a design argument that our layered use of \gls{hard immutability}, \gls{operational immutability}, and \gls{soft immutability} (following Vppalapati's taxonomy) removes or constrains the write surface an attacker could use.  Neither argument, by itself, is a substitute for measurement.

The clearest limitation, already anticipated in Section \ref{sec:scope-of-work}'s discussion of Assumptions, Risks, and Limitations, is that this is a single-system case study built by the same team that designed it, with no adversarial red-team validation and no comparison against a non-amnesiac control.  A second limitation is scope: our MITRE ATT\&CK mapping was produced by manual review rather than a reproducible, independently verifiable methodology, so a reader should treat the counts and categorizations in Tables \ref{tab:mitrewrite1}--\ref{tab:mitrewrite6} as illustrative rather than authoritative.  A third limitation, noted in the Literature Review, is that we have not empirically compared physically enforced immutability (our optical-media approach) against the software-only immutability used by Tails, immutable container nodes, or Silverblue/NixOS; it remains an open question whether the physical-enforcement layer meaningfully changes outcomes against a realistic attacker, or whether it mainly adds operational cost (as Vppalapati's framework would predict for any additional immutability layer).

Positioning this work relative to prior art, our contribution is most accurately described as a threat-model-driven design pattern for building an amnesiac server on FreeBSD using commodity \gls{zfs}, GELI, and \gls{jail} tooling, rather than a novel immutability mechanism.  The novelty, such as it is, lies in combining a physically enforced read-only \gls{os} layer with software-enforced \gls{zfs}-based separation of mutable working storage, and in explicitly mapping that combination back onto a defined subset of the MITRE ATT\&CK Enterprise matrix.  Future work should (1) construct a non-amnesiac control system on comparable hardware, (2) run a defined subset of the MITRE ATT\&CK techniques from Tables \ref{tab:mitrewrite1}--\ref{tab:mitrewrite6} against both systems under controlled conditions, (3) measure recovery time after a simulated \gls{ransomware} event on both systems, and (4) have the technique-to-hypothesis mapping reviewed by a second, independent analyst to check for classification bias.  Only with that evidence in hand would H1--H4 be adequately supported for a peer-reviewed research venue; in its current form, this chapter is best read as a reproducible engineering case study and threat-model argument that motivates, but does not yet demonstrate, those claims.
