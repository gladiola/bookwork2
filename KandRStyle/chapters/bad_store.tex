...
- PDF of Nessus scan using the Basic Network scan type\endnote{\url{https://github.com/gladiola/blackmagic/blob/Demo/agkistrodon/contortix/Penelope/BadStore/Nessus/salvage13_BasicNetwork_BadStore_ya6pkz.pdf}}
- PDF of Nessus scan using the Web Application scan type\endnote{\url{https://github.com/gladiola/blackmagic/blob/Demo/agkistrodon/contortix/Penelope/BadStore/Nessus/salvage13_BadStore_Web_e1yrt4.pdf}}

We clicked around some to look up those vulnerabilities on hyperlinks related to the Tenable website; while well supported with CVE documentation and the like, there were easier to exploit possibilties likely.  We continued with some of the other vuln scans like \texttt{nmap}.
...
\texttt{<100X>:<USER>/<PASSWORD>/<?OPTIONAL METAL WORD>/<IP ADDRESS>}
...
Meanwhile, it also turned up another script, \texttt{frmvrfy.js} that compares two password values.  Apparently part of the reset routine.  This was a program I felt we should exploit with our Javascript skills, but we set it aside for the time being.  
...
Kill Chain Step & Attacking Action & Observation \\
\hline
Nessus Basic Network & Apache 1.3 and OpenSSL < 0.9 \\
...
Historic vulnerabilities uncovered \\
Nessus Web scan \\
Historic vulnerabilities uncovered \\
nmap \\
Age of server versions will affect later exploits \\
nikto \\
Stubbed-out directories and loose files with clues \\
Manual probing \\
Relationship of inputs to CGI program and initial input validation guesses \\
\hline
\end{tabular}
\end{table}
...
By running some MySQL auxilliaries with \texttt{msfconsole}, we were able to send SQL directly to the MySQL engine, dump schema, and \texttt{SELECT} a bunch of useful content.  One of the results was that we were able to get database records for usernames as email addresses (this particular website logs users in by email addy), passwords, bank account numbers, and detailed transaction information. 

By \texttt{SELECT}ing those email addresses and passwords from the userdb table, \texttt{msfconsole} was able to output a simple text file that could be trimmed and used as input for \texttt{hashcat}. 


\subsection{Cracking hashed passwords with hashcat}
When the passwords were downloaded, by outputting the SQL to a text file, the passwords were hashed with MD5.  That simply wouldn't do.  In order to get a readily usable copy of those passwords, we'd need to crack the hash.  Hello, \texttt{hashcat}.
...
One more thing happened:  when feeding hashes in to \texttt{hashcat}, they would disappear from console output after the first crack.  The answers would turn up in the potfile, below.  It became clear that when feeding a big block of hashes to crack to \texttt{hashcat}, it was important to plan for an out-of-order extraction of answers.  For example, the fifth hash input would not necessarily be the fifth-from-last hash in the potfile.  So, in the future, we would need to prepare more to prevent fumbling when getting the cracked password back out.  This time, the result set was so small that simple comparison was still practical.

\subsubsection{Where is the potfile?}
After the first console display of the hashes, it was hard to see the cracked passwords for a moment.  When \texttt{hashcat} cracks a hash, by design, it will probably not attempt to crack that hash again.  In this way, \texttt{hashcat} can run faster and faster; with only unresolved hashes remaining, it will have less and less search area to cover.  
...
As mentioned earlier, the entries in the \texttt{hashcat} potfile didn't seem to be in the expected order.  A careful comparison of the hashes put in and the hashes in the potfile showed that they were not printed in the expected order.  The sequence of the input file was not the same as the sequence of hashes in the potfile.  Since the hashes were there beside their cracked values, a common search or \texttt{grep} would yield the correct answer; just be careful about skimming down the file or jumping to the last hash plaintext value; the last value in the potfile does not necessarily represent the last hash provided to \texttt{hashcat} to crack.

As a noob, this might seem counterintuitive.  Why would \texttt{hashcat} not simply answer with the cracked hash in the order that it received one to crack?  Keep in mind, that this is kind of a tiny "toy" use case.  \texttt{hashcat} is meant for bigger work.  If you wanted to crack many, many hashes using some GPUs, then \texttt{hashcat} was built for you.  This simple set of block extractions was almost an edge case.
...
Kill Chain Step & Attacking Action & Observation \\
\hline
msfconsole mysql schema dump & Get database schema \\
msf mysql sql & Obtain users table with passwords \\
hashcat & Offline password cracking in bulk \\
...
Observation \\
\hline
msfconsole mysql schema dump \\
Missing INFORMATION_SCHEMA was influential; nonstandard table names \\
msf mysql sql \\
Simple calls yielded necessary information to impersonate any account \\
hashcat \\
Password cracking with wordlists in a reasonable time allowed use of accounts uncovered in previous steps. \\
SQLi \\
Injectable controls in almost all inputs and file upload dialogs; other inputs showed CGI filtering of URL-encoded values \\
\hline
\end{tabular}
\end{table}
...
To make our life easier, we can use Burp Suite's proxy readouts to show us what the submitted POST will look like. [12]  From there, we'll save a copy of that POST message to a file, alter it slightly, and use it alongside our \texttt{sqlmap} to give the script a way to get an exploit installed.  \[[9]\]\[[10]\]\[[11]\]\[[12]\]
...
Kill Chain Step & Attacking Action & Observation \\
\hline
sqlmap & Automated discovery of effective SQLi \\
POST forms intercepted with Burp & Changing expected information \\
POST forms edited and staged & Efficient reloading of altered data for repetitive attack attempts \\
sqlmap & Command line uploads of files \\
...
\hline
Burp and sqlmap \\
Bypass POST requirement to mimic expected form actions \\
File uploads and downloads \\
Understanding what was where \\
sqlmap scans to determine SQLi \\
Much more complex attack than if derived manually \\
\hline
\end{tabular}
\end{table}
...
exploit
\end{lstlisting}
\texttt{msfconsole} commands like these came in handy to reveal most of what we wanted to know about the database supporting the dynamic website. \[[27]\]  Given these simple utilities, we were able to get any table we wanted and to see the entire schema.  Some of the prefabricated scripts for attacking this older version of MySQL database didn't work well because of the lack of an \texttt{INFORMATION_SCHEMA} database and standardized tables to cover the meta-structure of the databaases; but, that didn't stop our ability to glean the information we needed using the techniques above and some simple manual processes.  
...
Kill Chain Step & Attacking Action & Observation \\
\hline
sqlmap uploads & Attempted uploads foiled; shell installation unlikely \\
User Accounts & Decrypted passwords and known user accounts permit impersonation of any user on the site \\
...
\hline
Manual input \\
Impersonate users and observe account differences \\
MySQL console \\
Questions arose about skip-grant-tables and db account permissions \\
sqlmap uploads \\
Began seeking alternatives to file installation \\
\hline
\end{tabular}
\end{table}
...
Kill Chain Step & Attacking Action & Observation \\
\hline
Analyze downloaded files & Observe critical values; notice missing or empty files \\
sqlmap & Fetch system files to learn about system accounts and file structure \\
sqlmap, msfconsole & All attempts to upload shellcode failed \\
Linux File System & Multiple attempts to discern possible foothold on the target box by downloading key files. \\
\hline
\end{tabular}
\end{table}
...
\hline
sqlmap downloads \\
Determined the two user accounts through /etc/passwd; generally observed system files \\
sqlmap, msfconsole uploads \\
Could not install a file or detect a running service besides the Perl CGI already there \\
\hline
\end{tabular}
\end{table}
...
Review of techniques to implement a simple attack with default settings with a metasploit module using the \texttt{msfconsole}. 
...
Review of techniques to implement Nessus scans, \texttt{nmap} scans, and \texttt{nikto} scans. 
...
Review of techniques to implement substitution of values sent with Burp Suite, using \texttt{sqlmap} for SQLi, and understanding local and remote file inclusion attacks.

\endnote{[4]}  _____.  "Bad Store 1.2.3," Vulnhub.  INTERNET:  \texttt{https://www.vulnhub.com/entry/badstore-123,41/}\endnote{\url{https://www.vulnhub.com/entry/badstore-123,41/}} 
...
\endnote{[6]} _____.  "What is a potfile?" Hashcat Wiki.  INTERNET:  \texttt{https://hashcat.net/wiki/doku.php?id=frequently_asked_questions#how_can_i_show_previously_cracked_passwords_and_output_them_in_a_specific_format_eg_emailpassword}\endnote{\url{https://hashcat.net/wiki/doku.php?id=frequently_asked_questions#how_can_i_show_previously_cracked_passwords_and_output_them_in_a_specific_format_eg_emailpassword}}

\endnote{[7]} _____.  "How can I show previously cracked passwords, and output them in a specific format (e.g. email:password)?" Hashcat Wiki.  INTERNET:  \texttt{https://hashcat.net/wiki/doku.php?id=frequently_asked_questions#how_can_i_show_previously_cracked_passwords_and_output_them_in_a_specific_format_eg_emailpassword}\endnote{\url{https://hashcat.net/wiki/doku.php?id=frequently_asked_questions#how_can_i_show_previously_cracked_passwords_and_output_them_in_a_specific_format_eg_emailpassword}}

\endnote{[8]}  _____.  "Hashcat reports "Status: Cracked", but did not print the hash value, and the outfile is empty. What happened?" Hashcat Wiki.  INTERNET: \texttt{https://hashcat.net/wiki/doku.php?id=frequently_asked_questions#hashcat_reports_statuscracked_but_did_not_print_the_hash_value_and_the_outfile_is_empty_what_happened}\endnote{\url{https://hashcat.net/wiki/doku.php?id=frequently_asked_questions#hashcat_reports_statuscracked_but_did_not_print_the_hash_value_and_the_outfile_is_empty_what_happened}}

\endnote{[9]} Abouzeid, Halim.  "From SQL Injection to WebShell"  RSA.  INTERNET:  \texttt{https://community.rsa.com/community/products/netwitness/blog/2017/04/10/from-sql-injection-to-webshell}\endnote{\url{https://community.rsa.com/community/products/netwitness/blog/2017/04/10/from-sql-injection-to-webshell}}
...
\endnote{[10]} _____.  INTERNET:  \texttt{https://w00troot.blogspot.com/2017/05/getting-reverse-shell-from-web-shell.html}\endnote{\url{https://w00troot.blogspot.com/2017/05/getting-reverse-shell-from-web-shell.html}}
Some reverse shell script suggestions for various systems.  

\endnote{[11]} _____.  INTERNET:  \texttt{https://www.darkmoreops.com/2014/08/28/use-sqlmap-sql-injection-hack-website-database/}\endnote{\url{https://www.darkmoreops.com/2014/08/28/use-sqlmap-sql-injection-hack-website-database/}}
A tutorial showing some \texttt{sqlmap} hacks by injection.  Includes another example of get a hashed password and crack it using a modified form of \texttt{hashcat}.

\endnote{[12]} _____.  INTERNET:  \texttt{https://sneakerhax.com/tool-tips-sqlmap-with-post-requests/}\endnote{\url{https://sneakerhax.com/tool-tips-sqlmap-with-post-requests/}}
...
\endnote{[13]}  INTERNET:  \texttt{https://dev.mysql.com/doc/refman/5.5/en/create-user.html}\endnote{\url{https://dev.mysql.com/doc/refman/5.5/en/create-user.html}}
...
\endnote{[14]}  INTERNET:  \texttt{https://dev.mysql.com/doc/refman/5.5/en/grant.html}\endnote{\url{https://dev.mysql.com/doc/refman/5.5/en/grant.html}}
Commands for GRANT with MySQL.  Critical basic commands for setting privileges on an account with MySQL.

\endnote{[15]}  INTERNET:  \texttt{https://stackoverflow.com/questions/1708409/how-to-start-mysql-with-skip-grant-tables}\endnote{\url{https://stackoverflow.com/questions/1708409/how-to-start-mysql-with-skip-grant-tables}}
Stack Overflow post suggesting that skip grant tables might be overridden with reconfiguring some ini files.

\endnote{[16]}  INTERNET:  \texttt{https://superuser.com/questions/1127299/how-to-restart-mysql-with-skip-grant-tables-if-you-cant-use-the-root-password}\endnote{\url{https://superuser.com/questions/1127299/how-to-restart-mysql-with-skip-grant-tables-if-you-cant-use-the-root-password}}
...
\endnote{[17]}  INTERNET:  \texttt{http://seclists.org/metasploit/2012/q3/40}\endnote{\url{http://seclists.org/metasploit/2012/q3/40}}
Got turned on to \texttt{spool} for Metasploit commands.

\endnote{[18]}  INTERNET: \texttt{http://www.guninski.com/modproxy1.html}\endnote{\url{http://www.guninski.com/modproxy1.html}}
Post on \texttt{mod_proxy}

\endnote{[19]}  INTERNET: \texttt{https://hack2rule.wordpress.com/2017/02/25/sql-injection-to-meterpreter/}\endnote{\url{https://hack2rule.wordpress.com/2017/02/25/sql-injection-to-meterpreter/}}
Blog post on using \texttt{sqlmap}, \texttt{Meterpreter}, \texttt{Burp} and others.

\endnote{[20]}  INTERNET: \texttt{https://github.com/sqlmapproject/sqlmap/wiki/Usage}\endnote{\url{https://github.com/sqlmapproject/sqlmap/wiki/Usage}}
\texttt{sqlmap} github site explaining the usage of the arguments; man pages.

\endnote{[21]}  INTERNET: \texttt{https://www.tutorialspoint.com/perl/perl_cgi.htm}\endnote{\url{https://www.tutorialspoint.com/perl/perl_cgi.htm}}
...
\endnote{[22]}  Kamthan, Pankaj.  INTERNET: \texttt{https://www.irt.org/articles/js184/#origins_consequences}\endnote{\url{https://www.irt.org/articles/js184/#origins_consequences}}
...
\endnote{[23]}  INTERNET: \texttt{https://httpd.apache.org/docs/2.4/howto/cgi.html}\endnote{\url{https://httpd.apache.org/docs/2.4/howto/cgi.html}}
Apache documentation on setting up CGI with Apache 2.4.

\endnote{[24]}  INTERNET: \texttt{https://www.techrepublic.com/article/get-it-done-installing-apache-web-server-on-linux/}\endnote{\url{https://www.techrepublic.com/article/get-it-done-installing-apache-web-server-on-linux/}}
...
\endnote{[25]}  INTERNET: \texttt{https://www.cybrary.it/forums/topic/how-to-bypass-error-403-forbidden/}\endnote{\url{https://www.cybrary.it/forums/topic/how-to-bypass-error-403-forbidden/}}
Forum posts on other people getting 403s.  No big surprises here; but some sound advice on a statement of the conditions.

\endnote{[26]}  INTERNET: \texttt{https://www.cybrary.it/2015/05/summarizing-the-five-phases-of-penetration-testing/}\endnote{\url{https://www.cybrary.it/2015/05/summarizing-the-five-phases-of-penetration-testing/}}
Description of the five phases of pentesting.

\endnote{[27]}  INTERNET: \texttt{https://www.offensive-security.com/metasploit-unleashed/scanner-http-auxiliary-modules/}\endnote{\url{https://www.offensive-security.com/metasploit-unleashed/scanner-http-auxiliary-modules/}}
...
\endnote{[28]}  INTERNET: \texttt{https://www.rapid7.com/db/modules/auxiliary/scanner/ssl/openssl_heartbleed}\endnote{\url{https://www.rapid7.com/db/modules/auxiliary/scanner/ssl/openssl_heartbleed}}
Rapid7's description of an openssl_hearbleed scanner in Metasploit.

\endnote{[29]}  INTERNET: \texttt{https://www.trojanhorsesecurity.com/shellshock}\endnote{\url{https://www.trojanhorsesecurity.com/shellshock}}
Trojan Horse Security's description of a Shellshock scanner and exploit in Metasploit.

\endnote{[30]}  INTERNET: \texttt{https://www.rapid7.com/db/modules/exploit/multi/http/apache_mod_cgi_bash_env_exec}\endnote{\url{https://www.rapid7.com/db/modules/exploit/multi/http/apache_mod_cgi_bash_env_exec}}
Rapid7's description of a Shellshock exploit in Metasploit.

\endnote{[31]}  INTERNET: \texttt{https://bytebin.wordpress.com/2011/04/25/how-to-check-debian-boot-log-messages/}\endnote{\url{https://bytebin.wordpress.com/2011/04/25/how-to-check-debian-boot-log-messages/}}
Blog post on where to find Debian boot logs.  Useful notes when we were trying to locate BadStore VM's boot-up information.

\endnote{[32]}  INTERNET: \texttt{https://www.cvedetails.com/vulnerability-list/vendor_id-185/product_id-316/version_id-31799/Mysql-Mysql-4.0.25.html}\endnote{\url{https://www.cvedetails.com/vulnerability-list/vendor_id-185/product_id-316/version_id-31799/Mysql-Mysql-4.0.25.html}}
Web page on some CVEs related to early versions of MySQL.

\endnote{[33]}  INTERNET: \texttt{https://packetstormsecurity.com/files/128447/Apache-mod_cgi-Bash-Environment-Variable-Code-Injection.html}\endnote{\url{https://packetstormsecurity.com/files/128447/Apache-mod_cgi-Bash-Environment-Variable-Code-Injection.html}}
Blog post outlining the source code for a custom Metasploit module to work against Apache mod_cgi Bash.

\endnote{[34]}  INTERNET: \texttt{https://www.digitalocean.com/community/tutorials/how-to-use-passwd-and-adduser-to-manage-passwords-on-a-linux-vps}\endnote{\url{https://www.digitalocean.com/community/tutorials/how-to-use-passwd-and-adduser-to-manage-passwords-on-a-linux-vps}}
Tutorial on \texttt{/etc/passwd} that helped us understand what we saw when we downloaded a copy from BadStore.

\endnote{[35]}  INTERNET: \texttt{https://www.tldp.org/LDP/sag/html/adduser.html}\endnote{\url{https://www.tldp.org/LDP/sag/html/adduser.html}}
Linux Documentation Project's page on how to add a user.

\endnote{[36]}  INTERNET: \texttt{https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-115.pdf}\endnote{\url{https://nvlpubs.nist.gov/nistpubs/Legacy/SP/nistspecialpublication800-115.pdf}}
