...
%This template is tuned for a compact, classic look: narrow typeblock, Ga\gls{ram}ond body, and monospaced code.
...
So, there is a \gls{jail} for functional groups like:  development tools, offensive and defensive security prog\gls{ram}s, browsers, desktop software, and so on.  \gls{poudriere} jails can be filled with whatever sets of prog\gls{ram}s we like.  For successful builds, we find its best to keep the jails small.  
...
\begin{table} \centering \caption{ZFS SLOG Log Use SSD: Initial Estimates} \label{tab:zfsslogusedisk} \begin{tabular}{@{}p{0.60\linewidth} p{0.35\linewidth}@{}} \toprule & Estimate for 120\,GB SSD \\ \midrule Blank: 15\% "Free Space" & 18\,GB \\ Swap: 25\% Physical \gls{ram} (196\,GB max) & 50\,GB \\ ZFS Log Partition: Remainder & 52\,GB \\ \bottomrule \end{tabular} \end{table}
...
During the ssh-keygen dialog, simply direct the prog\gls{ram} to save the keys to \texttt{/tmp/otherDir/keys}.  Be sure to record your passphrase somewhere.  
...
\texttt{ \index{gpart!add}gpart add -a 1m -l ZFS\_NUMBER \textbackslash\\ -t freebsd-zfs DISK\_DEVICE}\footnote{Ibid.}\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -l ZFS-BLONDIE-4-WD123465789 \textbackslash\\-t freebsd-zfs  da4}\footnote{Ibid.}
\\

To add the partition's swap:\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -slg -l SWAP\_NUMBER \textbackslash\\-t freebsd-swap DISK\_DEVICE}\footnote{Ibid.}\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -slg -l SWAP-BLONDIE-4-WD123465789 \textbackslash\\-t freebsd-swap da4}\footnote{Ibid.}
\\

To add a logging partition on the SLOG SSDs:\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -l ZLOG\_NUMBER  \textbackslash\\-t freebsd-zfs DISK\_DEVICE}\footnote{Ibid.}\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -l ZLOG-BLONDIE-2-WD123465789 \textbackslash\\-t freebsd-zfs  da2}\footnote{Ibid.}\\\\
...
\begin{table} \centering \caption{ZFS SLOG Log Mirror (Write Cache) Disk — Initial Estimates} \label{tab:zfsslogmirror} \begin{tabular}{@{}p{0.60\linewidth} p{0.35\linewidth}@{}} \toprule & Estimate for 120\,GB SSD \\ \midrule GEOM Disk List Mediasize & 465\,GB \\ Blank: 15\% "Free Space" & 18\,GB \\ Swap: 25\% Physical \gls{ram} (196\,GB max) & 50\,GB \\ ZFS Log Partition: Remainder & 52\,GB \\ \bottomrule \end{tabular} \end{table}
...
Changed config to keep the RAIDZ2 for main work.  I was having trouble with the SSDs; so, I removed them and replaced them with two 80GB Western Digital Velociraptors.  I added another WD Velociraptor for cache.  Then we added a Western Digital Blue 500GB for a swap disk.  That sounds like a lot of swap, but we hope to upgrade to near 196GB of \gls{ram} this year. So, a little more than twice that would be 400GB of swap.  The next disk size on hand was 500GB.
...
\texttt{\index{gpart!add}gpart add -a 5K -s 395G -t freebsd-swap \textbackslash\\-l ZFS-BLONDIE-0-SWAP-WCC2ERZ56512 mfid0}\footnote{Ibid.}

\subsection{Create ZFS Cache in Bay 1}
\texttt{\index{gpart!create}gpart create -s GPT mfid1}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -a 5K -s 63G -t freebsd-zfs \textbackslash\\-l ZFS-BLONDIE-1-CACHE-WXL1E40C1352 mfid1}\footnote{Ibid.}

\subsection{Create ZFS SLOG in Bays 2 and 3}
\texttt{\index{gpart!create}gpart create -s GPT mfid2}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -a 5K -s 63G -t freebsd-zfs \textbackslash\\-l ZFS-BLONDIE-2-SLOG-WXL1E4067503 mfid2}\footnote{Ibid.}\\\\
\texttt{\index{gpart!create}gpart create -s GPT mfid3}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -a 5K -s 63G -t freebsd-zfs \textbackslash\\ -l ZFS-BLONDIE-3-SLOG-WXC0CA9V8431 mfid3}\footnote{Ibid.}\\\\

\subsection{Create ZFS Work Disks for Jails}
\subsection{Bay 4}
\texttt{\index{gpart!create}gpart create -s GPT mfid4}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -a 5K -s 50G -t freebsd-swap \textbackslash\\-l ZFS-BLONDIE-4-SWAP-WCC2EE134983 mfid4}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -s 345G -t freebsd-zfs \textbackslash\\-l ZFS-BLONDIE-4-FILES-WCC2EE134983 mfid4}\footnote{Ibid.}
\subsection{Bay 5}
\texttt{\index{gpart!create}gpart create -s GPT mfid5}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -a 5K -s 50G -t freebsd-swap \textbackslash\\-l ZFS-BLONDIE-5-SWAP-WMC2E3914420 mfid5}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -s 345G -t freebsd-zfs \textbackslash\\-l ZFS-BLONDIE-5-FILES-WMC2E3914420 mfid5}\footnote{Ibid.}
\subsection{Bay 6}
\texttt{\index{gpart!create}gpart create -s GPT mfid6}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -a 5K -s 50G -t freebsd-swap \textbackslash\\ -l ZFS-BLONDIE-6-SWAP-WCC2ETT39577 mfid6}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -s 345G -t freebsd-zfs \textbackslash\\-l ZFS-BLONDIE-6-FILES-WCC2ETT39577 mfid6}\footnote{Ibid.}
\subsection{Bay 7}
\texttt{\index{gpart!create}gpart create -s GPT mfid7}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -a 5K -s 50G -t freebsd-swap \textbackslash\\-l ZFS-BLONDIE-7-SWAP-WCAYU2878968 mfid7}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -s 345G -t freebsd-zfs \textbackslash\\-l ZFS-BLONDIE-7-FILES-WCAYU2878968 mfid7}\footnote{Ibid.}\\
...
\noindent\texttt{ \index{gpart!modify}gpart modify -i 1 \textbackslash\\-l ZFS-BLONDIE-4-SWAP-WCC2EE134983 mfid4}\footnote{Ibid.}\\\\
\texttt{ \index{gpart!modify}gpart modify -i 2 \textbackslash\\-l ZFS-BLONDIE-4-FILES-WCC2EE134983 mfid4}\footnote{Ibid.}\\
...
\texttt{ \index{gpart!add}gpart add -s 64G -t freebsd-swap \textbackslash\\-l ZFS-BLONDIE-0-SWAP-B-WCC2ERZ56512 mfid0}\footnote{“gpart(8).” Freebsd.org, 2026, \url{man.freebsd.org/cgi/man.cgi?query=gpart&apropos=0&sektion=8&manpath=FreeBSD+10.3-RELEASE&arch=default&format=html}. Accessed 10 June 2026.}\\\\
\texttt{\index{gpart!add}gpart add -s 64G -t freebsd-swap \textbackslash\\-l ZFS-BLONDIE-0-SWAP-C-WCC2ERZ56512 mfid0}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -s 64G -t freebsd-swap \textbackslash\\-l ZFS-BLONDIE-0-SWAP-D-WCC2ERZ56512 mfid0}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -s 64G -t freebsd-swap \textbackslash\\-l ZFS-BLONDIE-0-SWAP-E-WCC2ERZ56512 mfid0}\footnote{Ibid.}\\\\
\texttt{\index{gpart!add}gpart add -s 64G -t freebsd-swap \textbackslash\\-l ZFS-BLONDIE-0-SWAP-F-WCC2ERZ56512 mfid0}\footnote{Ibid.}\\\\
