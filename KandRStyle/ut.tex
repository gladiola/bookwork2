...
\begin{table}[H]
  ...
  \label{tab:cybercrime-anova-summary-1}
  \small
  \begin{tabular}{@{}p{0.18\linewidth}p{0.18\linewidth}p{0.18\linewidth}p{0.14\linewidth}p{0.22\linewidth}@{}}
    \toprule
    \textbf{Cybercrime Category} & \textbf{Object Type} & \textbf{Section Reference} & \textbf{Page Reference} & \textbf{Figure/Table Reference} \\
    \midrule
    ...
    \midrule
    Phishing & Lower Bound Computations & Section~\ref{sec:phishing} & page~\pageref{sec:phishing} & (subsection) \\
    & Upper Bound Computations & Section~\ref{sec:phishing} & page~\pageref{sec:phishing} & (subsection) \\
    & Figure for Monte Carlo simulation & Section~\ref{sec:phishing} & page~\pageref{fig:screenshot-montecarlo-phishing-count-discrete} & Figure~\ref{fig:screenshot-montecarlo-phishing-count-discrete} \\
    ...
    \midrule
    Ransomware & Lower Bound Computations & Section~\ref{sec:ransomware} & page~\pageref{sec:ransomware} & (subsection) \\
    & Upper Bound Computations & Section~\ref{sec:ransomware} & page~\pageref{sec:ransomware} & (subsection) \\
    & Figure for Monte Carlo simulation & Section~\ref{sec:ransomware} & page~\pageref{fig:screenshot-montecarlo-ransomware-count-histo} & Figure~\ref{fig:screenshot-montecarlo-ransomware-count-histo} \\
    & Table of simulation values & Section~\ref{sec:ransomware} & page~\pageref{sec:ransomware} & (see subsection) \\
    \midrule
...
\begin{table}[H]
  ...
  \label{tab:cybercrime-anova-summary-2}
  \small
  \begin{tabular}{@{}p{0.18\linewidth}p{0.18\linewidth}p{0.18\linewidth}p{0.14\linewidth}p{0.22\linewidth}@{}}
    \toprule
    \textbf{Cybercrime Category} & \textbf{Object Type} & \textbf{Section Reference} & \textbf{Page Reference} & \textbf{Figure/Table Reference} \\
    ...
    \bottomrule
  \end{tabular}
  \raggedright{\small Note: Each cybercrime category includes subsections for Lower Bound Computations and Upper Bound Computations derived from American Samoa (lower) and California (upper) data, scaled to the target population. Monte Carlo simulations were conducted with 10,000 iterations using appropriate distributions (discrete, binary, or normal) based on the data characteristics. The resulting figures and tables provide quantitative assessments of likelihood and impact for each category.}
\end{table}
...
Tables~\ref{tab:cybercrime-anova-summary-1} and~\ref{tab:cybercrime-anova-summary-2} provide a comprehensive reference guide for the quantitative analysis conducted on each cybercrime category in this chapter. These tables summarize the locations of lower bound computations, upper bound computations, Monte Carlo simulation figures, and simulation value tables that can be used for risk assessment after ANOVA validation.


\begin{table}[htbp] \centering \caption{Alignment of Retained Cybercrime Categories with MITRE ATT\&CK Techniques} \label{tab:attack_alignment} \begin{tabular}{|p{3.0cm}|p{3.5cm}|p{5.5cm}|} \hline \textbf{Category} & \textbf{ATT\&CK Tactic} & \textbf{Representative ATT\&CK Techniques} \\ \hline Personal Data Breach & Collection, Exfiltration & Data from Information Repositories (T1213), Archive Collected Data (T1560), Exfiltration Over Web Services (T1567) \\ \hline Phishing & Initial Access & Phishing (T1566), Phishing Attachment (T1566.001), Phishing Link (T1566.002), Spearphishing (T1566) \\ \hline Ransomware & Impact & Data Encrypted for Impact (T1486), Financial Theft (T1657) \\ \hline Extortion & Impact & Financial Theft (T1657), Data Encrypted for Impact (T1486), Data Leak Threats associated with double-extortion campaigns \\ \hline IPR / Counterfeit & Collection, Exfiltration & Data from Information Repositories (T1213), Exfiltration Over Web Services (T1567), Automated Collection (T1119) \\ \hline Identity Theft & Credential Access & Credentials from Password Stores (T1555), Steal or Forge Authentication Credentials (T1649), Brute Force (T1110) \\ \hline BEC & Initial Access, Credential Access, Impact & Phishing (T1566), Valid Accounts (T1078), Financial Theft (T1657), Account Manipulation (T1098) \\ \hline \end{tabular} \end{table}
