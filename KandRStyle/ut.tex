...
\begin{itemize}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item We'll use this as a RAIDZ-2 array to hold the jails.
		...
		\begin{itemize}
			...
			\begin{itemize}
				...
				\item We cannot give a GELI password through the console in our current setup.
			\end{itemize}	
	\end{itemize}
...
\begin{itemize}
	...
	\item a ZFS RAIDZ-2 kit of four drives.
\end{itemize}
...
\item A 4 disk RAIDZ-2 array of almost 1.49 TB of actual storage
...
\texttt{\# gpart create -s GPT da0}\\
\texttt{\# gpart add -t freebsd-ufs -a 1M da0}\\
...
We then note which drives we want to use for the main body of the RAIDZ-2 kit.  All of this will become one pool of ZFS VDEVs.  Then we'll add in the log mirrors.  So, in the sequence of 0-7, numbering the drives, we should expect to skip those two for now.
...
\texttt{gpart create -s SCHEME DISK\_DEVICE}\\\\
\texttt{gpart create -s gpt da4}
\\

To add the main ZFS partition:\\\\
\texttt{gpart add -a 1m -l ZFS\_NUMBER -t freebsd-zfs DISK\_DEVICE}\\\\
\texttt{gpart add -a 1m -l ZFS-BLONDIE-4-WD123465789 -t freebsd-zfs  da4}
\\

To add the partition's swap:\\\\
\texttt{gpart add -a 1m -slg -l SWAP\_NUMBER -t freebsd-swap DISK\_DEVICE}\\\\
\texttt{gpart add -a 1m -slg -l SWAP-BLONDIE-4-WD123465789 -t freebsd-swap da4}
\\

To add a logging partition on the SLOG SSDs:\\\\
\texttt{gpart add -a 1m -l ZLOG\_NUMBER -t freebsd-zfs DISK\_DEVICE}\\\\
\texttt{gpart add -a 1m -l ZLOG-BLONDIE-2-WD123465789 -t freebsd-zfs  da2}\\\\
...
\noindent\texttt{zpool create BLONDIE-MAIN raidz2 \textbackslash }\\
...
\texttt{gpart create -s GPT mfid0}\\\\
\texttt{gpart add -a 5K -s 395G -t freebsd-swap -l ZFS-BLONDIE-0-SWAP-WCC2ERZ56512 mfid0}

\subsection{Create ZFS Cache in Bay 1}
\texttt{gpart create -s GPT mfid1}\\\\
\texttt{gpart add -a 5K -s 63G -t freebsd-zfs -l ZFS-BLONDIE-1-CACHE-WXL1E40C1352 mfid1}

\subsection{Create ZFS SLOG in Bays 2 and 3}
\texttt{gpart create -s GPT mfid2}\\\\
\texttt{gpart add -a 5K -s 63G -t freebsd-zfs -l ZFS-BLONDIE-2-SLOG-WXL1E4067503 mfid2}\\\\
\texttt{gpart create -s GPT mfid3}\\\\
\texttt{gpart add -a 5K -s 63G -t freebsd-zfs -l ZFS-BLONDIE-3-SLOG-WXC0CA9V8431 mfid3}\\\\

\subsection{Create ZFS Work Disks for Jails}
\subsection{Bay 4}
\texttt{gpart create -s GPT mfid4}\\\\
\texttt{gpart add -a 5K -s 50G -t freebsd-swap -l ZFS-BLONDIE-4-SWAP-WCC2EE134983 mfid4}\\\\
\texttt{gpart add -s 345G -t freebsd-zfs -l ZFS-BLONDIE-4-FILES-WCC2EE134983 mfid4}
\subsection{Bay 5}
\texttt{gpart create -s GPT mfid5}\\\\
\texttt{gpart add -a 5K -s 50G -t freebsd-swap -l ZFS-BLONDIE-5-SWAP-WMC2E3914420 mfid5}\\\\
\texttt{gpart add -s 345G -t freebsd-zfs -l ZFS-BLONDIE-5-FILES-WMC2E3914420 mfid5}
\subsection{Bay 6}
\texttt{gpart create -s GPT mfid6}\\\\
\texttt{gpart add -a 5K -s 50G -t freebsd-swap -l ZFS-BLONDIE-6-SWAP-WCC2ETT39577 mfid6}\\\\
\texttt{gpart add -s 345G -t freebsd-zfs -l ZFS-BLONDIE-6-FILES-WCC2ETT39577 mfid6}
\subsection{Bay 7}
\texttt{gpart create -s GPT mfid7}\\\\
\texttt{gpart add -a 5K -s 50G -t freebsd-swap -l ZFS-BLONDIE-7-SWAP-WCAYU2878968 mfid7}\\\\
\texttt{gpart add -s 345G -t freebsd-zfs -l ZFS-BLONDIE-7-FILES-WCAYU2878968 mfid7}\\
...
\texttt{geli init -l 256 mfid0p1}\footnote{“Geli(8).” Freebsd.org, 2025, \url{man.freebsd.org/cgi/man.cgi?query=geli&sektion=8&manpath=FreeBSD+10.3-RELEASE}. Accessed 10 June 2026.}\\
\texttt{geli attach mfid0p1}\\\\
\texttt{geli init -l 256 mfid1p1}\\
\texttt{geli attach mfid1p1}\\\\
\texttt{geli init -l 256 mfid2p1}\\
\texttt{geli attach mfid2p1}\\\\
\texttt{geli init -l 256 mfid3p1}\\
\texttt{geli attach mfid3p1}\\\\
\texttt{geli init -l 256 mfid4p1}\\
\texttt{geli attach mfid4p1}\\\\
\texttt{geli init -l 256 mfid4p2}\\
\texttt{geli attach mfid4p2}\\\\
\texttt{geli init -l 256 mfid5p1}\\
\texttt{geli attach mfid5p1}\\\\
\texttt{geli init -l 256 mfid5p2}\\
\texttt{geli attach mfid5p2}\\\\
\texttt{geli init -l 256 mfid6p1}\\
\texttt{geli attach mfid6p1}\\\\
\texttt{geli init -l 256 mfid6p2}\\
\texttt{geli attach mfid6p2}\\\\
\texttt{geli init -l 256 mfid7p1}\\
\texttt{geli attach mfid7p1}\\\\
\texttt{geli init -l 256 mfid7p2}\\
\texttt{geli attach mfid7p2}\\\\
...
\texttt{WCC2EE134983} but that's not what we saw when we checked our work.  Not the end of the world.  We can move the .eli metadata file, and we can use gpart modify to change the label.\\

\noindent\texttt{gpart modify -i 1 -l \\ZFS-BLONDIE-4-SWAP-WCC2EE134983 mfid4}\\\\
\texttt{gpart modify -i 2 -l \\ZFS-BLONDIE-4-FILES-WCC2EE134983 mfid4}\\

\texttt{gpart show} and \texttt{geli status} should show our drives as we expect them to be.
...
\texttt{gpart resize -i 1 -s 64G mfid0}\\
...
\noindent\texttt{gpart recover mfid0}\\
\texttt{gpart status mfid0}\\
\texttt{geli attach mfid0p1}\\\\
\texttt{gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-B-WCC2ERZ56512 mfid0}\\\\
\texttt{gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-C-WCC2ERZ56512 mfid0}\\\\
\texttt{gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-D-WCC2ERZ56512 mfid0}\\\\
\texttt{gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-E-WCC2ERZ56512 mfid0}\\\\
\texttt{gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-F-WCC2ERZ56512 mfid0}\\\\


Then we need to establish encryption for all of those.\\

\noindent\texttt{geli init -l 256 mfid0p2}\\
\texttt{geli attach mfid0p2}\\
\texttt{geli init -l 256 mfid0p3}\\
\texttt{geli attach mfid0p3}\\
\texttt{geli init -l 256 mfid0p4}\\
\texttt{geli attach mfid0p4}\\
\texttt{geli init -l 256 mfid0p5}\\
\texttt{geli attach mfid0p5}\\
\texttt{geli init -l 256 mfid0p6}\\
\texttt{geli attach mfid0p6}\\
