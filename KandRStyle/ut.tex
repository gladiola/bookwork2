...
Formally, we note this process below.  $R$ represents our result.  $N_s$ represents our constant for state source population and is used for dividing the crime count into a per capita rate. $N_t$ represents our constant for target sample population and is used for multiplying the rate to scale it to match the population of the target. $t$ represents our starting year for the sample from \gls{icc} and coordinates the rows until it progresses to the final year chosen which is denoted by $n$. $i$ represents our chosen category index of cybercrime. $j$ represents our year index and is built with $t$ until it progresses to the final year of the sample denoted by $n$.

\[
R = \frac{N_t}{N_s} \sum_{j=t}^{t+6} M_{j,i}
\]

Two matrices inform this calculation.  The first, $P$, captures the population change for our target city over the years of the sample; it consists of a single column of population values indexed by year and has not been formally included here.  The second, $M$, organises the raw cybercrime counts from \gls{icc} by category and year:
...
The entries $a_{j,i}$ are the raw counts taken from the \gls{icc} state-level reports for the chosen state source.  Dividing by $N_s$ converts each count to a per-capita rate; multiplying by $N_t$ then scales that rate to the population of the target city.  Summing the selected column $i$ across all years yields the final per-capita-adjusted result $R$.
