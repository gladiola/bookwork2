...
We know that a standardized review of risk assessment categories can be found from NIST.  Their Cybersecurity Framework lists core components functions that include the topics of, "Identify, Protect, Detect, Respond, Recover."  \cite{NISTFramework}  For this next piece, I would suggest that a direct reference to the risk assessment categories in the reference would be helpful.  NIST provided a detailed and lengthy chart packed with information.  Below, we briefly consider chart components and points to concepts readily observed in the experiments under consideration.  This next paragraph will likely require a direct comparison to the table \cite{NISTFramework} to be helpful.
...
When we consider the possibility of responding to a SQL Injection attack by mitigating with an experimentally proven technique, we can see participation in all of these functions.  I find it helpful to consider them in reverse by supposing participation in a selected category of each function.  To "Recover": "Improvements" would have been agreed upon already, as part of accepting the concept of responsive security maintenance and mitigation.  To "Respond" : "Mitigation" is directly listed.  To "Detect" : "Security Continuous Monitoring" can be chosen; for example, even though SQL Injection attacks attempt to strike at the database, they are most likely to be detected by filtering and examining logs of calls made to the website through URLs.  This is why in the "Detect" function we are more likely to choose continuous monitoring over "Anomalies and Events."  To "Protect" : "Information Protection Processes \& Procedures" would be chosen because periodic log reviews would be a protection procedure.  Finally, to "Identify" : "Risk Assessment", or the identification, observation and response to risk from SQL Injection attacks would be chosen.  In this way, we can quickly relate the broader categories of the NIST Cybersecurity Framework to the immediate actions of mitigating SQLIATK.  This framework is useful to us as a paradigm for devising and implementing a mitigation response because it is a widely acknowledged emerging government standard for communicating information security concerns throughout the structure of entities like governments and businesses.  
 \cite{NISTFramework} \cite{NISTcybersecvid}
...
"Experiment: a study in which an intervention is deliberately introduced to observe its effects. \cite{Shadish12}"

"Randomized Experiment: An experiment in which units are assigned to receive the treatment or an alternative condition by a random process ... \cite{Shadish12}"

"Quasi-Experiment: An experiment in which units are not assigned to conditions randomly. \cite{Shadish12}"

"Natural Experiment: Not really an experiment because the cause usually cannot be manipulated; a study that contrasts a naturally occurring event such as an earthquake with a comparison condition. \cite{Shadish12}"

"Correlational Study: Usually synonymous with nonexperimental or observational study; a study that simply observes the size and direction of a relationship among variables. \cite{Shadish12}"

As Shadish discussed the features of a quasi-experiment, he noted some key ideas:

"Quasi-experiments share with all other experiments a similar purpose -- to test descriptive causal hypotheses about manipulatable causes ... \cite{ShadishText}(p. 14)"

"In quasi-experiments, the cause is manipulable and occurs before the effect is measured. \cite{Shadish14A}"

"In quasi-experiments, the researcher has to enumerate alternative explanations one by one, decide which are plausible, and then use logic, design, and measurement to assess whether each one is operating in a way that might explain any observed effect. \cite{Shadish14B}"

"Quasi-experimentation is falsificationist in that it requires experimenters to identify a causal claim and then to generate and examine plausible alternative explanations that might falsify the claim. \cite{Shadish15B}"

"Thus the focus on plausibility is a two-edged sword: it reduces the range of alternatives to be considered in quasi-experimental work, yet it also leaves the resulting causal inference vulnerable to the discovery that an implausible-seeming alternative may later emerge as a likely causal agent. \cite{Shadish17A}"
...
% Shadish Text
% Quotations from William Shadish's book, Experimental and Quasi-Experimental Designs for Generalized Causal Inference. \cite{ShadishText}

%REF Shadish 
\bibitem{Shadish2A}
Shadish, William, \cite{ShadishText}, pp. 2-3:
...
%REF Shadish 
\bibitem{Shadish3B}
Shadish, William, \cite{ShadishText}, p. 3 :
...
%REF Shadish 
\bibitem{Shadish4A}
Shadish, William, \cite{ShadishText}, p. 4:
...
%REF Shadish 
\bibitem{Shadish5A}
Shadish, William, \cite{ShadishText}, p. 5:
...
%REF Shadish 
\bibitem{Shadish6A}
Shadish, William, \cite{ShadishText}, p. 6:
...
%REF Shadish 
\bibitem{Shadish7A}
Shadish, William, \cite{ShadishText}, p. 7:
...
%REF Shadish 
\bibitem{Shadish9A}
Shadish, William, \cite{ShadishText}, p. 9:
...
%REF Shadish 
\bibitem{Shadish10A}
Shadish, William, \cite{ShadishText}, p. 10:
...
%REF Shadish 
\bibitem{Shadish11A}
Shadish, William, \cite{ShadishText}, p. 11:
...
%REF Shadish 
\bibitem{Shadish12A}
Shadish, William, \cite{ShadishText}, p. 12:
...
%REF Shadish 
\bibitem{Shadish14A}
Shadish, William, \cite{ShadishText}, p. 14:
...
%REF Shadish 
\bibitem{Shadish15A}
Shadish, William, \cite{ShadishText}, p. 15:
...
%REF Shadish 
\bibitem{Shadish16A}
Shadish, William, \cite{ShadishText}, p. 16:
...
%REF Shadish 
\bibitem{Shadish17A}
Shadish, William, \cite{ShadishText}, p. 17:
...
%REF Shadish p.18A
\bibitem{Shadish18A}
Shadish, William, \cite{ShadishText}, p.18.
...
%REF Shadish 
\bibitem{Shadish19A}
Shadish, William, \cite{ShadishText},p.19:
...
%REF Shadish 
\bibitem{Shadish20A}
Shadish, William, \cite{ShadishText}, p. 20:
...
%REF Shadish 
\bibitem{Shadish21A}
Shadish, William, \cite{ShadishText}, p. 21:
...
%REF Shadish 
\bibitem{Shadish22A}
Shadish, William, \cite{ShadishText}, p. 22:
...
%REF Shadish 
\bibitem{Shadish23A}
Shadish, William, \cite{ShadishText}, p. 23:
...
%REF Shadish 
\bibitem{Shadish24A}
Shadish, William, \cite{ShadishText}, p. 24:
...
%REF Lin
\bibitem{LinH99B}
Lin, Herbert, \cite{LinH}, p. 99-100:
...
%REF Lin
\bibitem{LinH105A}
Lin, Herbert, \cite{LinH}, p. 105-106: 
...
%REF Lin
\bibitem{LinH106B}
Lin, Herbert, \cite{LinH}, p. 106-107
...
%REF Lin
\bibitem{LinH108A}
Lin, Herbert, \cite{LinH}, p. 108-109
...
%REF OWASPZaw
\bibitem{OWASPZaw17A}
Watson, Collin and Tin Zaw, \cite{OWASPZaw}, p. 17:
...
Interview, Marcus Ranum and Wendy Nather, \cite{NatherRanum}:  
...
\bibitem{OWASPZaw8A}
Watson, Collin and Tin Zaw, \cite{OWASPZaw}, p. 8, Figure 1:
...
Shadish, William, \cite{ShadishProgEval} pp. 21-35.
...
Stallings, William. \cite{StallingsText}  p. 363
