...
Not having a root password may also truncate or stop our ability to use commands like \texttt{su} or \texttt{sudo}.  As we're trying to stop privilege escalation, we'll also be stopping legitimate means of priv-esc.  It could be inconvenient, but we've used this technique before.
...
\texttt{\#  \index{gpart}gpart create -s GPT da0}\\
\texttt{\#  \index{gpart}gpart add -t freebsd-ufs -a 1M da0}\\
\texttt{\# newfs -U /dev/da0p1}\\\\
Mount the USB to the /tmp Directory:\\\\
\texttt{\# mkdir /tmp/someDir}\\
\texttt{\# mount /dev/da0p1 /tmp/someDir}\\
...
\texttt{\# cd /tmp}\\
\texttt{\# umount /tmp/someDir}\\
\texttt{\# rm -r /tmp/someDir}\\\\
...
\texttt{\# mkdir /tmp/otherDir}\\
\texttt{\# mount /dev/da0p1 /tmp/otherDir}\\
\texttt{\# ls /tmp/otherDir}\\
...
We're going to choose to put a swap partition on the SSDs.  We can add one onto the other disks, but a maximum of 4 swap partitions is recommended in the \index{FreeBSD Handbook}FreeBSD Handbook.  Later on, we will be able to select swap partitions by mounting them and using commands like \texttt{swapon swapoff}.\\

To put the ZFS partition on the remaining disks:\\\\
\texttt{ \index{gpart}gpart create -s SCHEME DISK\_DEVICE}\\\\
\texttt{ \index{gpart}gpart create -s gpt da4}
\\

To add the main ZFS partition:\\\\
\texttt{ \index{gpart}gpart add -a 1m -l ZFS\_NUMBER -t freebsd-zfs DISK\_DEVICE}\\\\
\texttt{ \index{gpart}gpart add -a 1m -l ZFS-BLONDIE-4-WD123465789 -t freebsd-zfs  da4}
\\

To add the partition's swap:\\\\
\texttt{ \index{gpart}gpart add -a 1m -slg -l SWAP\_NUMBER -t freebsd-swap DISK\_DEVICE}\\\\
\texttt{ \index{gpart}gpart add -a 1m -slg -l SWAP-BLONDIE-4-WD123465789 -t freebsd-swap da4}
\\

To add a logging partition on the SLOG SSDs:\\\\
\texttt{ \index{gpart}gpart add -a 1m -l ZLOG\_NUMBER -t freebsd-zfs DISK\_DEVICE}\\\\
\texttt{ \index{gpart}gpart add -a 1m -l ZLOG-BLONDIE-2-WD123465789 -t freebsd-zfs  da2}\\\\

With each disk partitioned, we can add them to the pool for our RAIDZ-2 kit.\\\\
\texttt{zpool create RAID POOL\_NAME DISK\_LABEL/ZFS\_PARTITION\_LABEL ...next disk/partition...}\\\\

\noindent\texttt{zpool create BLONDIE-MAIN \index{RAIDZ-2}raidz2 \textbackslash }\\
...
\texttt{zpool add BLONDIE-MAIN log mirror \textbackslash}\\
...
\texttt{mkdir /tmp/usb3}\\
\texttt{mount /dev/da0p1 /tmp/usb3}
...
\texttt{\index{geli!init}geli init -l 256 mfid1p1}\\
\texttt{\index{geli!attach}geli attach mfid1p1}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid2p1}\\
\texttt{\index{geli!attach}geli attach mfid2p1}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid3p1}\\
\texttt{\index{geli!attach}geli attach mfid3p1}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid4p1}\\
\texttt{\index{geli!attach}geli attach mfid4p1}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid4p2}\\
\texttt{\index{geli!attach}geli attach mfid4p2}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid5p1}\\
\texttt{\index{geli!attach}geli attach mfid5p1}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid5p2}\\
\texttt{\index{geli!attach}geli attach mfid5p2}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid6p1}\\
\texttt{\index{geli!attach}geli attach mfid6p1}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid6p2}\\
\texttt{\index{geli!attach}geli attach mfid6p2}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid7p1}\\
\texttt{\index{geli!attach}geli attach mfid7p1}\\\\
\texttt{\index{geli!init}geli init -l 256 mfid7p2}\\
\texttt{\index{geli!attach}geli attach mfid7p2}\\\\
...
\noindent\texttt{cp /var/backups/mfid0p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-0-SWAP-WCC2ERZ56512.eli}\\\\
\texttt{cp /var/backups/mfid1p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-1-CACHE-WXL1E40C1352.eli}\\\\
\texttt{cp /var/backups/mfid2p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-2-SLOG-WXL1E4067503.eli}\\\\
\texttt{cp /var/backups/mfid3p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-3-SLOG-WXC0CA9V8431.eli}\\\\
\texttt{cp /var/backups/mfid4p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-4-SWAP-WCC2EE134983.eli}\\\\
\texttt{cp /var/backups/mfid4p2.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-4-FILES-WCC2EE134983.eli}\\\\
\texttt{cp /var/backups/mfid5p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-5-SWAP-WMC2E3914420.eli}\\\\
\texttt{cp /var/backups/mfid5p2.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-5-FILES-WMC2E3914420.eli}\\\\
\texttt{cp /var/backups/mfid6p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-6-SWAP-WCC2ETT39577.eli}\\\\
\texttt{cp /var/backups/mfid6p2.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-6-FILES-WCC2ETT39577.eli}\\\\
\texttt{cp /var/backups/mfid7p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-7-SWAP-WCAYU2878968.eli}\\\\
\texttt{cp /var/backups/mfid7p2.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-7-FILES-WCAYU2878968.eli}
...
\texttt{ \index{gpart!show}gpart show} and \texttt{geli status} should show our drives as we expect them to be.
...
\noindent\texttt{swapon mfid0p1.eli}
\texttt{swapinfo -k}\\
...
\noindent\texttt{swapon /dev/mfid4p1.eli}\\
\texttt{swapon /dev/mfid5p1.eli}\\
\texttt{swapon /dev/mfid6p1.eli}\\
\texttt{swapon /dev/mfid7p1.eli}\\
\texttt{swapinfo -h}\\
