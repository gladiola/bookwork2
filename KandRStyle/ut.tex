...
\begin{itemize}
  ...
  \item Open Data from local government agencies
  \item MITRE ATT\&CK for guiding data selection
  \item Other scientifically valid open data sources.
\end{itemize}
...
We want others to be able to conduct their own risk assessments 
...
\begin{table}[h!]
  \centering
  \caption{Changes to Population in Key Areas}
...
\begin{table}[h!]
  \centering
  \caption{Projected In-City Victim Counts, City of Chattanooga, Threats to System Intrusion}
...
\begin{figure}[h!]
  ...
  \caption{A Monte Carlo simulation suggests that there is a 90\% probability that 8 or more personal data breaches will occur within the sample.  There is a 50\% likelihood that 17 or more will occur.  There is a 10\% chance that 26 or more will occur.  Sample size was set equivalent to the City of Chattanooga to determine the per capita rates for each year over data from American Samoa and California.  The medians of those rates were used to set 10\% and 90\% confidence intervals with American Samoa and California representing low and high cybercrime counts respectively.
  \protect\endnote{Monte Carlo Modeling Toolkit authored by Derek E. Brink, CISSP, \url{www.linkedin.com/in/derekbrink}. March 2023.}
  }
...
\begin{figure}[h!]
  ...
  \caption{A Monte Carlo simulation suggests that there is a 90\% probability that personal data breaches will cost \$10,362 or more in a year.  There is a 50\% likelihood that they will cost \$52,724 or more in a year.  There is a 10\% chance that they will cost \$94,876 or more in a year.  Sample size was set equivalent to the City of Chattanooga to determine the per capita rates for each year over data from American Samoa and California.  The medians of those rates were used to set 10\% and 90\% confidence intervals with American Samoa and California representing low and high cybercrime impact values respectively.
  \protect\endnote{Monte Carlo Modeling Toolkit authored by Derek E. Brink, CISSP, \url{www.linkedin.com/in/derekbrink}. March 2023.}
  }
  \label{fig:screenshot-montecarlopersdatabreachexceedence}
\end{figure}
...
\begin{table}[h!]
  \centering
  \caption{Projected In-City Victim Counts, City of Chattanooga, Threats to Personnel}
...
\begin{table}[h!]
  \centering
  \caption{Projected In-City Victim Counts, City of Chattanooga, Theft of Data Via Network}
...
\subsubsection{Methodology for Monte Carlo Simulations in Excel}
We chose \gls{ic3} crime data from American Samoa to act as our 10\% confidence limit.  We chose data from Californiz to act as our 90\% confidence limit.  We set those limits by looking at seven year spans of crime data and taking the median.  This means we would be likely to have observations above and below the limits, but that the limits themselves would have a basis in observation.  
We input and processed the data as follows:
\begin{itemize}
\item We imported the data from\gls{ic3} crime tables
\item We calculated counts and costs for Chattanooga, American Samoa, and California.
\item We imported a Uniform distribution table from the Monte Carlo Toolkit.
\item We set median summary counts from American Samoa as our 10\% interval.
\item We set median summary counts from California as our 90\% interval.
\item We accepted the Monte Carlo percentages and exceedence curves.
\end{itemize}
