...
\begin{itemize}
	\item A DVD RW drive
    	\item A LTO III tape backup unit
    	\item In her 8 drive bay, we have:
	\begin{itemize}
		\item Two Velociraptor 80GB 10000 rpm drives.
		\begin{itemize}
		...
		\end{itemize}
		\end{itemize}
		\item We replaced the one CPU with two Xeon 5550 quad cores and added a cooling tower.
...
\begin{table}
  ...
  \begin{tabular}{@{}p{0.18\linewidth} p{0.30\linewidth} p{0.48\linewidth}@{}}
    \toprule
    Topic & Concept & Command phrase example \\
    \midrule
    Disk labels & Use the label to associate the logical with the physical &
      \texttt{[Host]-[PositionNumber 0-7]-[Serial\_Number]} \\
    ZFS Compression & Faster performance: less space &
      \texttt{zfs set compress=gzip-6 [DIRECTORY]} \\
    ZFS zpool comment & Make a note on the pool &
      \texttt{zfs set comment="-"} \\
    quota & Limit growth & --- \\
    ...
    canmount & Control mounting & \texttt{zfs set canmount=off [DIRECTORY]} \\
    mountpoint & Specify where to mount & \texttt{zfs set mountpoint=/SOME [DIRECTORY]} \\
    readonly & Control readonly & \texttt{zfs set readonly=off [DIRECTORY]} \\
    exec & Control if it can execute & \texttt{zfs set exec=off [DIRECTORY]} \\
    copies & Make more than one copy & \texttt{zfs set copies=2 [DIRECTORY]} \\
    \bottomrule
  \end{tabular}
\end{table}

After reading over \index{FreeBSD Mastery:  ZFS}FreeBSD Mastery:  ZFS by Alan Jude and Michael Lucas, we came to a few conclusions.  We chose to use \index{RAIDZ-2}RAIDZ-2 with Log drives because of the number of drives we have.  Some of the topics covered in that book that we'll want to apply include ideas in Table \ref{tab:zfsconcepts}.  In our first iterations we decided to install the OS on the hard drives in a pattern that looks like a conventional installation.  Once the drive partitioning and utilitization commands were worked out, we then started over and undertook the \index{amensiac}amnesiac version with the OS on a live disc that runs with attached drives.  
...
\texttt{\index{cd}cd /tmp}\\
\texttt{\index{umount}umount /tmp/someDir}\\
\texttt{\index{rm} rm -r /tmp/someDir}\\\\
...
\texttt{\index{ls} ls /tmp/otherDir}\\
...
\texttt{ \index{gpart!create}gpart create -s SCHEME DISK\_DEVICE}\\\\
\texttt{ \index{gpart!create}gpart create -s gpt da4}
\\

To add the main ZFS partition:\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -l ZFS\_NUMBER -t freebsd-zfs DISK\_DEVICE}\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -l ZFS-BLONDIE-4-WD123465789 -t freebsd-zfs  da4}
\\

To add the partition's swap:\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -slg -l SWAP\_NUMBER -t freebsd-swap DISK\_DEVICE}\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -slg -l SWAP-BLONDIE-4-WD123465789 -t freebsd-swap da4}
\\

To add a logging partition on the SLOG SSDs:\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -l ZLOG\_NUMBER -t freebsd-zfs DISK\_DEVICE}\\\\
\texttt{ \index{gpart!add}gpart add -a 1m -l ZLOG-BLONDIE-2-WD123465789 -t freebsd-zfs  da2}\\\\

With each disk partitioned, we can add them to the pool for our RAIDZ-2 kit.\\\\
\texttt{\index{zpool!create}zpool create RAID POOL\_NAME DISK\_LABEL/ZFS\_PARTITION\_LABEL ...next disk/partition...}\\\\

\noindent\texttt{\index{zpool!create}zpool create BLONDIE-MAIN \index{RAIDZ-2}raidz2 \textbackslash }\\
...
\texttt{\index{zpool!add}zpool add BLONDIE-MAIN log mirror \textbackslash}\\
...
\texttt{\index{mount}mount /dev/da0p1 /tmp/usb3}

\subsection{Create Swap Disk in Bay 0}
\texttt{ \index{gpart!create}gpart create -s GPT mfid0}\\\\
\texttt{ \index{gpart!add}gpart add -a 5K -s 395G -t freebsd-swap -l ZFS-BLONDIE-0-SWAP-WCC2ERZ56512 mfid0}

\subsection{Create ZFS Cache in Bay 1}
\texttt{ \index{gpart!create}gpart create -s GPT mfid1}\\\\
\texttt{ \index{gpart!add}gpart add -a 5K -s 63G -t freebsd-zfs -l ZFS-BLONDIE-1-CACHE-WXL1E40C1352 mfid1}

\subsection{Create ZFS SLOG in Bays 2 and 3}
\texttt{ \index{gpart!create}gpart create -s GPT mfid2}\\\\
\texttt{ \index{gpart!add}gpart add -a 5K -s 63G -t freebsd-zfs -l ZFS-BLONDIE-2-SLOG-WXL1E4067503 mfid2}\\\\
\texttt{ \index{gpart!create}gpart create -s GPT mfid3}\\\\
\texttt{ \index{gpart!add}gpart add -a 5K -s 63G -t freebsd-zfs -l ZFS-BLONDIE-3-SLOG-WXC0CA9V8431 mfid3}\\\\

\subsection{Create ZFS Work Disks for Jails}
\subsection{Bay 4}
\texttt{ \index{gpart!create}gpart create -s GPT mfid4}\\\\
\texttt{ \index{gpart!add}gpart add -a 5K -s 50G -t freebsd-swap -l ZFS-BLONDIE-4-SWAP-WCC2EE134983 mfid4}\\\\
\texttt{ \index{gpart!add}gpart add -s 345G -t freebsd-zfs -l ZFS-BLONDIE-4-FILES-WCC2EE134983 mfid4}
\subsection{Bay 5}
\texttt{ \index{gpart!create}gpart create -s GPT mfid5}\\\\
\texttt{ \index{gpart!add}gpart add -a 5K -s 50G -t freebsd-swap -l ZFS-BLONDIE-5-SWAP-WMC2E3914420 mfid5}\\\\
\texttt{ \index{gpart!add}gpart add -s 345G -t freebsd-zfs -l ZFS-BLONDIE-5-FILES-WMC2E3914420 mfid5}
\subsection{Bay 6}
\texttt{ \index{gpart!create}gpart create -s GPT mfid6}\\\\
\texttt{ \index{gpart!add}gpart add -a 5K -s 50G -t freebsd-swap -l ZFS-BLONDIE-6-SWAP-WCC2ETT39577 mfid6}\\\\
\texttt{ \index{gpart!add}gpart add -s 345G -t freebsd-zfs -l ZFS-BLONDIE-6-FILES-WCC2ETT39577 mfid6}
\subsection{Bay 7}
\texttt{ \index{gpart!create}gpart create -s GPT mfid7}\\\\
\texttt{ \index{gpart!add}gpart add -a 5K -s 50G -t freebsd-swap -l ZFS-BLONDIE-7-SWAP-WCAYU2878968 mfid7}\\\\
\texttt{ \index{gpart!add}gpart add -s 345G -t freebsd-zfs -l ZFS-BLONDIE-7-FILES-WCAYU2878968 mfid7}\\
...
\noindent\texttt{\index{cp}cp /var/backups/mfid0p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-0-SWAP-WCC2ERZ56512.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid1p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-1-CACHE-WXL1E40C1352.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid2p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-2-SLOG-WXL1E4067503.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid3p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-3-SLOG-WXC0CA9V8431.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid4p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-4-SWAP-WCC2EE134983.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid4p2.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-4-FILES-WCC2EE134983.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid5p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-5-SWAP-WMC2E3914420.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid5p2.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-5-FILES-WMC2E3914420.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid6p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-6-SWAP-WCC2ETT39577.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid6p2.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-6-FILES-WCC2ETT39577.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid7p1.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-7-SWAP-WCAYU2878968.eli}\\\\
\texttt{\index{cp}cp /var/backups/mfid7p2.eli /tmp/usb3/backups\\\_27SEP2020/ZFS-BLONDIE-7-FILES-WCAYU2878968.eli}
...
\texttt{WCC2EE134983} but that's not what we saw when we checked our work.  Not the end of the world.  We can move the .eli metadata file, and we can use  \index{gpart!modify}gpart modify to change the label.\\

\noindent\texttt{ \index{gpart!modify}gpart modify -i 1 -l \\ZFS-BLONDIE-4-SWAP-WCC2EE134983 mfid4}\\\\
\texttt{ \index{gpart!modify}gpart modify -i 2 -l \\ZFS-BLONDIE-4-FILES-WCC2EE134983 mfid4}\\

\texttt{ \index{gpart!show}gpart show} and \texttt{\index{geli!status}geli status} should show our drives as we expect them to be.
...
\noindent\texttt{\index{swapon}swapon mfid0p1.eli}
\texttt{\index{swapinfo}swapinfo -k}\\
...
\texttt{ \index{gpart!resize}gpart resize -i 1 -s 64G mfid0}\\
...
\noindent\texttt{ \index{gpart!recover}gpart recover mfid0}\\
\texttt{ \index{gpart!status}gpart status mfid0}\\
\texttt{\index{geli!attach}geli attach mfid0p1}\\\\
\texttt{ \index{gpart!add}gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-B-WCC2ERZ56512 mfid0}\\\\
\texttt{\index{gpart!add}gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-C-WCC2ERZ56512 mfid0}\\\\
\texttt{\index{gpart!add}gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-D-WCC2ERZ56512 mfid0}\\\\
\texttt{\index{gpart!add}gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-E-WCC2ERZ56512 mfid0}\\\\
\texttt{\index{gpart!add}gpart add -s 64G -t freebsd-swap -l \\ZFS-BLONDIE-0-SWAP-F-WCC2ERZ56512 mfid0}\\\\


Then we need to establish encryption for all of those.\\

\noindent\texttt{\index{geli!init}geli init -l 256 mfid0p2}\\
\texttt{\index{geli!attach}geli attach mfid0p2}\\
\texttt{\index{geli!init}geli init -l 256 mfid0p3}\\
\texttt{\index{geli!attach}geli attach mfid0p3}\\
\texttt{\index{geli!init}geli init -l 256 mfid0p4}\\
\texttt{\index{geli!attach}geli attach mfid0p4}\\
\texttt{\index{geli!init}geli init -l 256 mfid0p5}\\
\texttt{\index{geli!attach}geli attach mfid0p5}\\
\texttt{\index{geli!init}geli init -l 256 mfid0p6}\\
\texttt{\index{geli!attach}geli attach mfid0p6}\\

using \texttt{swapinfo}, \texttt{\index{swapon}swapon} and \texttt{swapoff}, we were able to remove some of the other swap partitions and add on these.  Did see a kernel limit on swap.  Total configured swap exceeded kern.maxswzone (98087976 pages).  We could tune the kernel, or we could reduce swap.  For now, we reduce swap from 384GB to 324GB.
...
\noindent\texttt{\index{swapon}swapon /dev/mfid4p1.eli}\\
\texttt{\index{swapon}swapon /dev/mfid5p1.eli}\\
\texttt{\index{swapon}swapon /dev/mfid6p1.eli}\\
\texttt{\index{swapon}swapon /dev/mfid7p1.eli}\\
\texttt{swapinfo -h}\\
...
\noindent\texttt{mkdir /tmp/BLONDIE}\\
