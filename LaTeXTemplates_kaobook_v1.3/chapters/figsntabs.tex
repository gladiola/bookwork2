...
\index{Figures}Figures and \index{tables}tables can be inserted just like in any standard 
\LaTeX\xspace document. The \Package{graphicx} \index{package}package is already loaded 
and configured in such a way that the \index{figure width}figure width is equal to the 
textwidth and the \index{height}height is adjusted in order to maintain the original 
\index{aspect ratio}aspect ratio. As you may have imagined, the \index{captions}captions will be 
positioned\ldots well, in the \index{margins}margins. This is achieved with the help of 
the \Package{floatrow} \index{package}package.

Here is a picture of Mona Lisa (\reffig{normalmonalisa}), as an example. 
The \index{captions}captions are formatted as the \index{margin-notes}margin- and the \index{side-notes}side-notes; If you 
want to change something about \index{captions}captions you can use the \index{command}command 
\Command{captsetup} from the \Package{caption} \index{package}package. Remember that if 
you want to \index{reference}reference a \index{figure}figure, the \index{label}label must come \emph{after} the 
...
While the format of the \index{caption}caption is managed by \Package{caption}, its 
\index{position}position is handled by the \Package{floatrow} \index{package}package. Achieving this 
result has been quite hard, but now I am pretty satisfied. In two-side 
mode, the \index{captions}captions are printed in the correct \index{margin}margin.

\index{Tables}Tables can be inserted just as easily as \index{figures}figures, as exemplified by the 
following code:
...
\index{Marginfigures}Marginfigures can be inserted with the \index{environment}environment 
\Environment{marginfigure}. In this case, the whole picture is confined 
to the \index{margin}margin and the \index{caption}caption is below it. \reffig{marginmonalisa} is 
...
There is also the \Environment{margintable} \index{environment}environment, of which 
\reftab{anotheruseless} is an example. Notice how you can place the 
\index{caption}caption above the \index{table}table by just placing the \Command{caption} \index{command}command 
before beginning the \Environment{tabular} \index{environment}environment. Usually, \index{figure captions}figure 
captions are below, while \index{table captions}table captions are above. This rule is also 
respected for normal \index{figures}figures and \index{tables}tables: the \index{captions}captions are always on the 
side, but for \index{figure}figure they are aligned to the bottom, while for \index{tables}tables to 
the top.
