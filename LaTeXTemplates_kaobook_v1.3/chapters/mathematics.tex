...
Despite most people complain at the sight of a book full of \index{equations}equations, 
\index{mathematics}mathematics is an important part of many books. Here, we shall 
illustrate some of the possibilities. We believe that \index{theorems}theorems, 
\index{definitions}definitions, \index{remarks}remarks and \index{examples}examples should be emphasised with a shaded 
\index{background}background; however, the colour should not be to heavy on the eyes, so 
we have chosen a sort of light yellow.\sidenote{The \index{boxes}boxes are all of the 
...
As you may have noticed, \index{definitions}definitions, \index{example}example and \index{remarks}remarks have 
independent \index{counters}counters; \index{theorems}theorems, \index{propositions}propositions, \index{lemmas}lemmas and \index{corollaries}corollaries 
share the same \index{counter}counter.
...
There is also an \index{environment}environment for \index{exercises}exercises.

\begin{exercise}
Prove (or disprove) the Riemann hypothesis.
\end{exercise}

We provide one \index{package}package for the \index{theorem styles}theorem styles: 
\href{kaotheorems.sty}{kaotheorems.sty}, to which you can pass the 
\Option{framed} \index{option}option you do want coloured \index{boxes}boxes around \index{theorems}theorems, like 
in this document.\sidenote{The \index{styles}styles without \Option{framed} are not 
showed, but actually the only difference is that they don't have the 
yellow \index{boxes}boxes.} You may want to edit this files according to your taste 
and the general \index{style}style of the book. However, there is an \index{option}option to 
\index{customise}customise the \index{background colour}background colour of the \index{boxes}boxes if you use the 
\Option{framed} \index{option}option: when you load this \index{package}package, you can pass it the 
\Option{background=mycolour} \index{option}option (replace \enquote{mycolour} with the 
actual colour, for instance, \enquote{red!35!white}). This will change 
the colour of all the \index{boxes}boxes, but it is also possible to override the 
default colour only for some elements. For instance, the 
\Option{propositionbackground=mycolour} \index{option}option will change the colour 
for \index{propositions}propositions only. There are similar \index{options}options for theorem, 
...
a \index{box}box in these cases. We used \Package{mdframed} to construct the ones 
shown below. You can create and modify such \index{environments}environments by editing the 
