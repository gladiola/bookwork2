...
use the \Command{sidecite}\index{\Command{sidecite}} \index{command}command. It does 
not have an \index{offset}offset argument yet, but it probably will in the future. 
This \index{command}command supports multiple entries, as you can see, and by default 
it prints the \index{reference}reference on the \index{margin}margin as well as adding it to the 
\index{bibliography}bibliography at the end of the document. Note that the \index{citations}citations have 
...
For this setup I wrote a separate \index{package}package, \Package{kaobiblio}, which 
you can find in the \Package{styles} directory and include in your main 
tex file. This \index{package}package accepts all the \index{options}options that you can pass to 
\Package{biblatex}, and actually it passes them to \Package{biblatex} 
under the hood. Moreover, it also defines some \index{commands}commands, like 
\Command{sidecite}, and \index{environments}environments that can be used within a 
...
and the \index{options}options are exactly the same.}
...
\index{reference}reference of the available \index{commands}commands can be found in this
\href{http://tug.ctan.org/info/biblatex-cheatsheet/biblatex-cheatsheet.pdf}{cheatsheet},
under the \enquote{Citations} section.

Finally, to \index{compilation}compile a document containing \index{citations}citations, you need to use an 
external tool, which for this class is \index{biber}biber. You need to run the 
...
\index{glossary}
The \Class{kaobook} \index{class}class loads the \index{packages}packages \Package{glossaries} and 
\Package{imakeidx}, with which you can add \index{glossaries}glossaries and \index{indices}indices to 
your book. For instance, I previously defined some \index{glossary entries}glossary entries and 
now I am going to use them, like this: \gls{computer}. 
\Package{glossaries} also allows you to use \index{acronyms}acronyms, like the 
following: this is the full version, \acrfull{fpsLabel}, and this is the 
short one \acrshort{fpsLabel}. These \index{entries}entries will appear in the \index{glossary}glossary 
in the \index{backmatter}backmatter.
...
\index{index}
To create an \index{index}index, you need to insert the \index{command}command 
\lstinline|\index{subject}| whenever you are talking about 
\enquote{subject} in the text. For instance, at the start of this 
paragraph I would write \lstinline|index{index}|, and an \index{entry}entry would be 
added to the Index in the \index{backmatter}backmatter. Check it out!

\marginnote[2mm]{In theory, you would need to run an external \index{command}command 
for the \index{index}index as well, but luckily the \index{package}package we suggested, 
	\Package{imakeidx}, can \index{compilation}compile the \index{index}index automatically.}

\index{nomenclature}
A \index{nomenclature}nomenclature is just a special kind of \index{index}index; you can find one at the end of
this book. To insert a \index{nomenclature}nomenclature, we use the \index{package}package \Package{nomencl} and
add the terms with the \index{command}command \Command{nomenclature}. We put then a
