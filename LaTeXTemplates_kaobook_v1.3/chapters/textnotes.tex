...
\index{Sidenotes}Sidenotes are a distinctive feature of all 1.5-column-\index{layout}layout books. 
Indeed, having wide \index{margins}margins means that some material can be displayed 
there. We use \index{margins}margins for all kind of stuff: \index{sidenotes}sidenotes, \index{marginnotes}marginnotes, 
small tables of contents, \index{citations}citations, and, why not?, special \index{boxes}boxes and 
\index{environments}environments.

\section{Sidenotes}

\index{Sidenotes}Sidenotes are like \index{footnotes}footnotes, except that they go in the \index{margin}margin, where 
they are more readable. To insert a \index{sidenote}sidenote, just use the \index{command}command 
\Command{sidenote\{Text of the note\}}. You can specify a 
mark\sidenote[O]{This sidenote has a special mark, a big O!} with \\ 
\Command{sidenote[mark]\{Text\}}, but you can also specify an \index{offset}offset, 
which moves the \index{sidenote}sidenote upwards or downwards, so that the full syntax is:
...
As we said, \index{sidenotes}sidenotes are handled through the \Package{sidenotes} 
\index{package}package, which in turn relies on the \Package{marginnote} \index{package}package.

\section{Marginnotes}

This \index{command}command is very similar to the previous one. You can create a 
\index{marginnote}marginnote with \Command{marginnote[offset]\{Text\}}, where the \index{offset}offset 
argument can be left out, or it can be a multiple of 
\Command{baselineskip},\marginnote[-1cm]{While the \index{command}command for \index{margin notes}margin 
notes comes from the \Package{marginnote} \index{package}package, it has been redefined 
in order to change the position of the optional \index{offset}offset argument, which 
...
\Command{baselineskip} as \index{offset}offset. These things were made only to make 
